      PROGRAM gDBWS00
!modification of 9807
!this program will read the control file in a free format, the first data value 
!on each line. these data are tagged with comments leading by underscore
...
C    AVAILABLE FROM PROF. R. A. YOUNG AT THE ABOVE ADDRESS.
      include 'param.inc'
!only one set of parameters are used here, with 32000 data points and 
...
!updating data file after cycle is also changed
      REAL LAMDA
      INTEGER PTR
...
c     COMMON changed  !cp ap 97
     ...
     * ,IPREF,IABSR,FONDO,IAS,iasym,sw,ibgd,idata
...
c     * ,IPREF,IABSR,FONDO,IAX,IAS,CTIME,FI,iasym,sw,iaxx,ibgd
     ...
     *  PTR(NATS),RELAX(4),RATIO(2)
      ...
      COMMON/JNK/ALOW(30),AHIGH(30),POS(30),BCK(30),
...
c unit 4=data file
!changed the default files, for de
!     open (4,file=' ',status='unknown')
	open (4,file ="pw_dbws.dat", status='unknown')
c unit 5 Input control File      
!      open (5,file=' ',status='unknown')
	open (5,file="pw_icf.txt",status='unknown')
c unit 6 = output file      
!      open (6,file=' ',status='unknown')
	open(6,file="pw_dbws.out", status='unknown')
c unit 7 = r indexes      
...
c      PARAMETER (idsz=2048 ,irs=512,NATS=64,msz=32 ,nov=128)
      COMMON/G1/THMIN,STEP,THMAX,U,V,W,LAMDA(2),TMV,CTHM,RLIM,BKPOS,WDT
     * ,ULOR,VLOR,ZZZ,UC
      COMMON/G2/S1,S2,ss2,S3,S4,ss4,D1,D2,D4,R1,R2,R3
      COMMON/DATAX/Y(IDSZ),VAR(IDSZ),YC(IDSZ),KR(IDSZ)
     * ,BK(IDSZ),NPTS,AMORPHOUS(IDSZ)
...
1400        CONTINUE
      ...
      S2=S2+DEL*DEL/VAR(I)
      ss2 = ss2 + del*del
      IF (DELWP.LT.1.0E+24) S4=S4+(DELW-DELWP)*(DELW-DELWP)
...
3         Cmod=Cmod+DABS(A(I,J))
      DPINV=AMAX1(DPINV,Cmod)      !there is a problem with this C, 
!&Fortran VF considers this as a KIND definition
        M(I)=I
        B(I)=B(I)/U(I)
        DO 2 J=1,N
2         A(I,J)=A(I,J)/U(I)
...
C                           USE IN SYMOPR TO DEFINE SITE SYMMETRY.
      ...
      COMMON /HKLCTL/ IHKL(3,24),AZ(24),NC1(10,7,99),ICHKL(99),
     1    N1HKL(99),IER
C     INTEGER AND
C     AND(I,J) = IAND(I,J)
      K = 2
...
C                           TO THE INPUT HKL PHASE.
     ...
     1    N1HKL(99),IER
      ...
      DIMENSION NCH(24),NCK(24),NCL(24) 
      DIMENSION LD(3,24)
      DIMENSION IPH(24)
...
c      WRITE(*,9752)NAm(NS),DFP(NS),DFPP(NS), XMAS(NS),lamda(1)
      WRITE (6,9752)NAm(NS),DFP(NS),DFPP(NS), XMAS(NS),lamda(1)
9752  FORMAT(' ATOM =',a4,1X,'dfp,dfpp=',2(F10.5,1X),' MASS=',
     *F8.3, '  Wlenght:',f8.6)
c      WRITE(*,9751) (AC(IJI,NS),IJI=1,9) 
      WRITE(6,9751) (AC(IJI,NS),IJI=1,9) 
9751  FORMAT(' Scat Coef.=', 9(F9.5,1X)) 
...
1076  B = A
      C = A
      GO TO 1072
1002  COSA = COS1(ALPHA/360.0)
105   COSB = COS1(BETA/360.0) 
1052  COSC = COS1(GAMMA/360.0)
...
8730   FORMAT (1X, 'NO. OF PHASESZ', 2X,I4,/,1X,
     *  ' NO. OF REFLECTIONS IN EACH PHASEQ',2X,99I4)
       WRITE(9,8731)
...
c  below is to correct the Fobs and Fcal for alpha_2 !cp 24.maio.98
c          if(irc.eq.1) then
c                  xlor=1.0
c                   else
c                  xlor=ratio(2)
...
c        Line below also from italian code   !cp ap 97 
        ...
        IF(IPC.EQ.1) WRITE(6,482)
        IF(IPC.EQ.2) WRITE(6,8482)
        IF(IPC.EQ.3) WRITE(6,8483)
        END IF
482     FORMAT(32H0NO.  &
     &CODE    H   K   L   HW     ,16HPOSN      ICALC ,4X,5HIOBS )
...
9221      if(mod(ixxx-1,60).eq.0) then
          if(ipc.eq.1) write(6,9481)
          if(ipc.eq.2) write(6,9483)
          if(ipc.eq.3) write(6,9485)
        end if
...
9222      CONTINUE
        IF(MOD(IXXx-1,60).EQ.0) THEN
        IF (IPC.EQ.1) WRITE(6,9482)
        IF (IPC.EQ.2) WRITE(6,19482)
        if (ipc.eq.3) write(6,29482)
        END IF    
...
8476  FORMAT (1X,'NPTSZ',I5,/, 1X, 'THMINZ',F8.3,/,1X,'STEPZ',F8.3,/,
     *1X,4HYOBS,4X,6HYCALCZ)
      DO 31 I=1,NPTS
      BK(I)=THMIN+FLOAT(I-1)*STEP
31    WRITE(9,8477)Y(I),YC(I)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
!11rewriting... 
!adapted to write a easier format,...
...
c      COMMON/AIRPAR/AI1,AI2,AI3,AI4,AI5,AI6,AI7,AI8,AI9,AI10
c      EQUIVALENCE (GLB(17),SCAIR),(GLB(18),PMON1)
c      EQUIVALENCE (GLB(19),PMON2),(GLB(20),SCAM)
c      EQUIVALENCE (AGLB(17),FLSCAIR),(AGLB(18),FLMON1)
c      EQUIVALENCE (AGLB(19),FLMON2),(AGLB(20),FLSCAM)
       REWIND 5
C line 1
      WRITE(5,1)TITLE
1     FORMAT(A70)
      ...
      NPROF=NPROF-1 
      INSTRM = INSTRM-1
C      WRITE(5,2)JOBTYP,NPROF,NPHASE,ibckcode,NEXCRG,NSCAT,INSTRM,IPREF,
C     1          iasym,ias,iabsr
C2     FORMAT(11I4,13x,'LINE 2.1')
C      if(ibckcode.eq.-1)write(5,202)fondo,iaxx
C202   format(2i4,49x,'LINE 2.2')
c line 2.1
      WRITE(5,2)JOBTYP,NPROF,NPHASE,ibckcode,NEXCRG,NSCAT,INSTRM,IPREF,
     1          iasym,iabsr,idata
2     FORMAT(11I4)
c line 2.2
      if(ibckcode.eq.-1)write(5,202)IAS,fondo
202   format(2i4)
C      WRITE(5,14)IOT,IPL,IPC,MAT,NXT,LST1,LST2,LST3,IPL1,IPL2,IPLST,
C     *IPLOSS,IPLCAL,IPLPOL,IPLCOM,IPLDIS,IPLAM,IPLAIR,IPBIG
C14    FORMAT(3(5i1,1x),4i1,35x,'LINE 3')
c line 3
...
46    FORMAT(6(f9.2,1x),3x,/,6(F9.4,1x)) 
...
465     FORMAT(A50)
c line 11-2
      WRITE(5,48)NATOM(K),NMOL(K), saqf(k),(PREF(K,I),I=1,3)
48      FORMAT(2I4,2x,f8.4,2x,f5.3,2x,f5.2,2x,f5.3)
      N=NATOM(K)
c line 11-3
...
!     *            I=1,N)
	write (5,6511) atext(i+iof)
	write (5,6512) murt(i+iof)
	write (5,6511) ntyp(i+iof)
6511	format(a4)		      
6512	format(i4)
		  WRITE(5,65) ((XL(I+IOF,J),J=1, 5),(A(I+IOF,J),J=1, 5),
...
65    FORMAT(f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x
     *    ,f9.4,2x,/,
     *    10x,f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x
     *    ,/,f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x,1X,/,
     *    f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x,f9.4,2x)
...
50      FORMAT(G8.3,F8.4,41X,/,F8.2,2x,f8.2/,
     *        7(F8.5,1X),1x,/,7(F8.2,1x)/,
     *        6(F8.4,2x),9X,/,6(F8.2,2x)/,
     *        3(F8.5,2x),2x,/,3(F8.2,2x),/,
     *        3(F8.4,2x),2x,/,3F8.2,/,
     1        3(F8.4,2x),2x,/,3F8.2,/,
     1         f8.4,4x,/,F8.2)
81      CONTINUE
...
C-----MONOCHROMATOR PARAMETERS DERIVATIVES
      ...
      ESSE=2*SIN((TH-GLB(1)-SHIFT)*0.00872665)/LAMBDAM
      LK=LGLB(18)
      IF ( LK.NE.0 ) THEN
...
6001  WRITE(6,24) IRS
      WRITE(7,24) IRS
24    FORMAT (//' TOO MANY REFLECTIONS, INCREASE *IRS* IN THE PARAMETER
...
C                 AIR COMPOSITION 20% O2 AND 80% N2 AT 300øK AND 1 ATM.:
...
C
      END
!!here is reading the input data
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
!!
!
...
!
!
!reading data, modified to free format by ND, december 2000
...
c      CHARACTER TITLE*70,PHSNM(8)*50
      CHARACTER TITLE*70,PHSNM(99)*50, chjunk*20 !chjunk*20 is used for text
...
c1     FORMAT(BZ,A70)
1     FORMAT(A70)
c      READ(5,2,END=99999)JOBTYP,NPROF,NPHASE,NBCKGD,NEXCRG,NSCAT,INSTRM,
c     *IPREF,fondo,iax,ias,IABSR,iasym       
C !cp jun 97 iax --> iaxx, changed to acomodate the ibcg flag to not evaluate
c            amorphous, air contributions
C      READ(5,2,END=99999)JOBTYP,NPROF,NPHASE,NBCKGD,NEXCRG,NSCAT,INSTRM,
C     *IPREF,fondo,iaxX,ias,IABSR,iasym   
C*** changes in the read format. !cp jul 97
c line 2.1
c      READ(5,2,END=99999)JOBTYP,NPROF,NPHASE,NBCKGD,NEXCRG,NSCAT,INSTRM,
c     *IPREF,iasym,ias,IABSR
c line 2
!format 2 neglected, december 2000
      READ(5,*,END=99999)JOBTYP,NPROF,NPHASE,NBCKGD,NEXCRG,NSCAT,INSTRM,
     *    IPREF,iasym,IABSR,idata   
		
!all the following lines were removed, these were used for the conversion
!of a dbws94 to dbws98 control file...I don't think it is really useful
!in a GUI program
!              if(nprof.lt.0) then 
!                  icnvt=1
!                  write(6,8000)
!      write(*,8000)
!      call conv94
!there might also be a problem with the calling convention here
...
3     FORMAT('1RIETVELD ANALYSIS PROGRAM DBWS-9807, RELEASE ',
     *' 21.09.98,',/,'   COPYRIGHT 1998 BY R.A.YOUNG,',/,
...
6     FORMAT(26H PATTERN CALCULATION, XRAY  )
      IF(JOBTYP.EQ.4)WRITE(6,7)
7     FORMAT(29H PATTERN CALCULATION, NEUTRON  )
...
12    FORMAT(39H BACKGROUND CORRECTION BY INTERPOLATION  ,
     * 12HBETWEEN THE  ,I4,12HPOINTS GIVEN )
...
c line 2.2
! it was 202 in read instruction
          read(5,*,END=99999)ias, fondo
202     FORMAT(BZ,2I4)
        endif
...
5333      if (ibgd.eq.1) then
          write(6,7447)
7447      format(' NO AMORPHOUS AND COMPTON CORRECTION TO THE BGD')
          goto 7448    
        end if
...
c14    FORMAT(BZ,19I1)
c line 3
! format card 14 replaced here with *, december 2000
7448  READ(5,*,END=99999)IOT,IPL,IPC,MAT,NXT,LST1,LST2,LST3,IPL1,IPL2
...
c line 4
! * instead of 23
      READ(5,*,END=99999)LAMDA,RATIO(2),BKPOS,WDT,CTHM,TMV,RLIM,sw
23    FORMAT(BZ,9F8.0)
C        !cp ap 21 97
      RATIO(1) = 1.0
...
c line 6(*)
!* instead of 34
      READ(5,*,END=99999)(POS(I),BCK(I),I=1,NBCKGD)
34    FORMAT(BZ,2F8.2)
...
120   IF(NEXCRG.LE.0)GOTO 122 
c line 7(*)
!* instead of 34, dec 2000
      READ(5,*,END=99999)(ALOW(I),AHIGH(I),I=1,NEXCRG)
...
c line 8.1 XRD(*)
!* for 3838, dec 2000
         READ(5,*,END=99999)NAM(I),DFP(I),XMAS(I) 
3838       FORMAT(BZ,A4,2F8.0)
         GOTO 125
       ENDIF
c line 8.1 ND(*)
!*/38, dec 2000
      READ(5,*,END=99999)NAM(I),DFP(I),DFPP(I),XMAS(I) 
38      FORMAT(BZ,A4,3F8.0)
      K=0
c line 8.2 XRD(*)
!*/39, dec 2000
126     READ(5,*,END=99999)(AC(J,I),J=1,9)
39      FORMAT(BZ,9F8.0)
...
c line 10.1
!*/45, dec 2000
      READ(5,*,END=99999)glb(1),glb(10),glb(11),glb(8),glb(9),glb(12),
     *                    glb(13) 
c line 10.11
!*/45, dec 2000
      READ(5,*,END=99999)aglb(1),aglb(10),aglb(11),aglb(8),aglb(9),
     1                    aglb(12),aglb(13)
45    FORMAT(BZ,10F8.0)
...
!*/455, dec 2000
         READ(5,*,END=99999)glb(20), glb(18), glb(19),
...
471      FORMAT(BZ,3F8.2,A56)                        
c read DBWS formated
!here is the data file, formated read is OK
...
c read sCINTAG data file (read start,stop,step and data)
      ...
      if(idata.eq.1.or.idata.eq.2)read(4,*,END=99998)(Y(I),I=1,NPTS)
472   FORMAT(BZ,8(F7.0,1X))
3333  DO 4484 I=1,NPTS
...
C     TAUK = 2.4621E-6
      DO 520 I=NPTS+1,IPTS+NPTS
        ...
        FLEET2= (FLEET2*STPTIM*(1-TAUK*Y(I)))**2.
        VAR(I) = (STPTIM*Y(I))/FLEET2 
        Y(I) = (Y(I)/(1-TAUK*Y(I))-OFSTI1)*CHMBRI/FLEET1
...
c line 11.2
!*/48, dec 2000
      READ(5,*,END=99999)NATOM(K),NMOL(K),saqf(k),(PREF(K,I),I=1,3)
      N=NATOM(K)
         IF (saqf(k).LE.0.)THEN
               saqf(k)=1.0
               write(*,478)k
...
!*/65, dec 2000
      READ(5,*,END=99999)(ATEXT(I+IOF),murt(I+IOF),NTYP(I+IOF),
     *      (XL(I+IOF,J),J=1, 5), (A(I+IOF,J),J=1, 5),
     *      (XL(I+IOF,J),J=6,11), (A(I+IOF,J),J=6,11),I=1,N)
...
!*/50
      READ(5,*,END=99999) PAR(K,1),PAR(K,2),                   !S  O_B (line 11-5)
     *                          APAR(K,1),APAR(K,2),
...
c checking for zeros if TCH-PV is being used
      if(nprof.eq.8)then
         if(par(k,15).eq.0)par(k,15)=1e-6
         if(par(k,16).eq.0)par(k,16)=1e-6
      end if
...
774       format('  LOW SIDE EXPONENT COEFFICIENTS=',3F12.4,/,
     ...
     1             '  SPLIT PEARSON VII ASYMMETRY PARAMETER=',3F12.4,/)
C-----IF NOT THE SPLIT PEARSON VII PROFILE
       else
       write(6,779) PAR(K,17),PAR(K,18),PAR(K,19)
779      format(' MIXING PARAMETERS = ',3G8.3,/)
       end if
...
781     format(' MIXING PARAMETERS = ',3F8.2,/)
      end if
...
C     CHECK FOR NON-ZERO COUNTS AT ALL POINTS
      DO 5678 I=1,NPTS
      IF (VAR(I).LE.1.0E-6) THEN
        ANGLEP = THMIN + FLOAT(I-1) * STEP
        WRITE (6,5679) I,ANGLEP
        WRITE (7,5679) I,ANGLEP
5679   FORMAT (5X,'ZERO COUNTS AT STEP NO.',I7,'  AT TWO THETA ',F8.3)
        STOP '*** ZERO COUNTS IN DATA FILE AT A STEP ***' 
      ENDIF
5678    CONTINUE
      RETURN
...
c         print '(5x,3f10.5,i3)',prexpx,precorx,dprecorx,ij     ! ******
        END DO
!       write(6,'(a,4i3,a,i2)') 'hkl',(ihkl(i,1),i=1,3),nprof,' iph',iph  !ACL debug 980520
...
C PREPARING PHASE and struc fact TO BE PRINTED  !cp june 98
      ...
      if(AV.lt.0.and.BV.GT.0)aphase(nn)=aphase(nn)+3.1415927359
      if(AV.lt.0.and.BV.lt.0)aphase(nn)=aphase(nn)+3.1415927359
      if(AV.lt.0.and.BV.eq.0)aphase(nn) =-3.1415927359
      if(AV.gt.0.and.BV.eq.0)aphase(nn) =0.00
...
c       size of FINAL is too small in case of many atoms !!!
     ...
     * ,IPREF,IABSR,FONDO,IAS,iasym,sw,ibgd,idata
      ...
      COMMON/COEFF/AC(10,16),POSI(30),SCAT(30),DFP(16),DFPP(16),xmas(16)
      COMMON/MULTIP/TMASSA(99),MLTPHASE,xmltp(99),MURT(NATS),SAQF(99)
      DIMENSION SY(30),SZ(30),DCSM(6,6),DCV(6),DUMMY(2*MSZ+4)
...
3       FORMAT(7H0PHASE ,I2,2H: ,A50,/' NEW PARAMETERS, SHIFTS, AND ',
     *     'STANDARD DEVIATIONS='//5H ATOM,5X,1HX,6X,2HDX,5X,2HSX,6X,1HY
     *   ,6X, 2HDY,5X,2HSY,6X,1HZ,6X,2HDZ,5X,2HSZ,6X,1HB,6X,2HDB,5X,2HSB
...
c !cp jun 96 stop
100     FORMAT(1X,A4,2X,5(3F7.4,1X))
      WRITE(6,101)
...
105        format('0MIXING PARAMETERS ',/,' NA= ',3g10.4,/
     *                                    ' NB= ',3g10.4,/,
     *                                    ' NC= ',3g10.4,/)
       end if
...
c Print comand below is for testing the codes. !cp 13junho98
...
1884  FORMAT( ' AMORPHOUS SCALE (SCAM):',3F8.4)
c !cp ap 20 97   !from It. codes
      WRITE(6,1882) (GLB(J),SY(J),SZ(J),J=18,19)
1882  FORMAT (/' MONOCROMATOR BANDPASS PARAMETERS (PMONI) ',/,9F8.4)
...
48      FORMAT(BZ,2I4,8x,3F4.0)
      saqf(k) = 1.
      write(6,820)NATOM(K),NMOL(K), saqf(k),(PREF(K,I),I=1,3)
820   FORMAT(2I4,f8.4,3F4.2,29X,'#ATMS #FU AFQPA PREFDIR')
c
      N=NATOM(K)
c line 11.3
      READ(5,460,END=99999)(SYMB(K,I),I=1,20)
...
110   format(2(a10,1x),f8.3)  !a delimiter 'comma' was added here,
30     read(4,1)test
      do i=1 ,56
...
c6     FORMAT(8(F8.0,1X))
      write(97,62)thmin,step,thmax,dataid
62    format(3f8.3,a56)      
