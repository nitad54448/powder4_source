...
If Not (txtMerge(j).Text = "") Then
    ...
    Open inputfile For Input As inpfil
        ...
        Input #inpfil, a, b ', c ; just xy, bug hunted 17.06.2002
...
If mergeFile(j + 1) Then
    ...
    Do While Not (EOF(inpfil))
        For i = 1 To numvalori(j + 1)
            ...
            'Zt2(j + 1, i) = (Yt(j + 1, i) / normFactor(j + 1)) 'square
        Next i
    Loop
    Close #inpfil
...
If mergeFile(j) Then
    ...
    Do While Not (EOF(inpfil))
        For i = 0 To numvalori(j) - 1
            ...
            Zt2(j, i + pointerX(j)) = Yt(j, i + pointerX(j)) / normFactor(j)
...
raport "X, Y, ESD ascii file written; one record per line..." & vbCrLf & Now
Exit Sub
errorTRAP:
raport Err.Description
Err.Clear
Close
raport "An error has occured, routine : merge XY files, at  :" & CStr(Now)
Exit Sub
End Sub
